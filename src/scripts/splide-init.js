...
(function(){
  ...
  function mergeConfigs(defaults, custom){
    ...
    if(defaults.breakpoints || (custom && custom.breakpoints)) {
      merged.breakpoints = Object.assign({}, defaults.breakpoints || {}, (custom && custom.breakpoints) || {});
    }
    return merged;
  }
  ...
  function initSplide(el) {
    ...
    if(dataType === 'gallery-with-thumbs') {
      ...
      const thumbs = el.nextElementSibling && el.nextElementSibling.querySelector('.splide') ? el.nextElementSibling.querySelector('.splide') : null;
      if(thumbs) {
        const main = new Splide(el, config);
        const thumbConfig = mergeConfigs({ rewind: true, fixedWidth: 100, fixedHeight: 60, isNavigation: true, gap: '8px', perPage: 4 }, (parsed && parsed.thumbs) || {});
   ...
   splideInstance = new Splide(el, config);
    ...
    // Mount with window.splide.Extensions to enable AutoScroll
    ...
    for(const m of mutations) {
      for(const node of m.addedNodes) {
        if(node.nodeType === 1) {
          ...
          else if(node.querySelector && node.querySelector('.splide')) initAllSplides(node);
        }
      }
    }
  });
